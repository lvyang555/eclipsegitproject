...
<script type="text/javascript">
	function del(id) {
		...
		if (flag) {
			window.location.href = "DeleteRoute?id=" + id;
		}
	}
	function update(id) {
		window.location.href = "UpdateRoute.jsp?id=" + id ;
	}
	function query(id) {
		window.location.href = "QueryStation.jsp?id=" + id ;
	}
</script>
</head>
<body>
...
<table align="center" border="1" width="560px" cellspacing="0">
		...
		<tr>
			...
			<td>
				...
				<a href="javascript:update(<%=routes.get(i).getRid()%>)">修改</a>
				<a href="javascript:del(<%=routes.get(i).getRid()%>)">删除</a></td>
		</tr>
		<%
			}
		%>
	...
	<center>
		...
		<%
			...
			if (currentPage > 1) {
		%>
		<a href="RouteInfo.jsp.jsp?page=1">第一页</a> <a
			href="RouteInfo.jsp.jsp?page=<%=currentPage - 1%>">上一页</a>
		<%
			}
			if (currentPage < maxPage) {
		%>
		<a href="RouteInfo.jsp?page=<%=currentPage + 1%>">下一页</a> <a
			href="RouteInfo.jsp?page=<%=maxPage%>">最后一页&nbsp;</a>
		<%
			}
		%>
	</center>
